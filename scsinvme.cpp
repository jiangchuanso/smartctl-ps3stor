...
{
public:
  ps3stornvme_device(smart_interface * intf, scsi_device * scsidev,
                    const char * req_type, unsigned nsid,
                    ctrl_id_t cid, unsigned did);

  virtual ~ps3stornvme_device();
...
ps3stornvme_device::ps3stornvme_device(smart_interface * intf, scsi_device * scsidev,
                                     const char * req_type, unsigned nsid,
                                     ctrl_id_t cid, unsigned did)
: smart_device(intf, scsidev->get_dev_name(), "", req_type),
  tunnelled_device<nvme_device, scsi_device>(scsidev, nsid),
  m_cid(cid),
  m_did(did)
{
  set_info().info_name = strprintf("%s [ps3stor_disk_%02d]", scsidev->get_dev_name(), m_did);  
}

ps3stornvme_device::~ps3stornvme_device()
{
}

bool ps3stornvme_device::nvme_pass_through(const nvme_cmd_in & in, nvme_cmd_out & out)
{
  ps3stor_nvme_encapsution_req_t req              = {};
  ps3stor_nvme_encapsution_rsp_t rsp              = {};
  ps3stor_nvme_cmd_t             nvme_cmd         = {};

  //copy for nvme_cmd
  nvme_cmd.cdw0  = in.opcode;
  nvme_cmd.nsid  = in.nsid;
  nvme_cmd.cdw10 = in.cdw10;
  nvme_cmd.cdw11 = in.cdw11;
  nvme_cmd.cdw12 = in.cdw12;
  nvme_cmd.cdw13 = in.cdw13;
  nvme_cmd.cdw14 = in.cdw14;
  ...
  req.encapsulatedCmdLength = sizeof(nvme_cmd);
  memcpy(req.encapsulatedNVMeCmd, &nvme_cmd, sizeof(nvme_cmd));
  req.submissionQueueType = PS3STOR_NVME_SUBMISSIONQ_ADMIN;

  //call ps3stor nvme interface
  ps3stor_errno err = ps3libNVMePassthru(m_cid, &req, &rsp, in.buffer, in.size);
  ...
  //out.result = ? todo fill out.result
  return true;
}
