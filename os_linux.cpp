...
public:
  linux_ps3stor_device(smart_interface *intf, const char *name,
    unsigned int tgt, unsigned int ctrlId);
...
private:
  ...
  slot_id_t              m_sid;  
  unsigned int           m_host;
  ps3stor_pd_interface_e m_dev_interface;
  unsigned int           m_nsd;// for nvme

  bool scsi_cmd(scsi_cmnd_io *iop);
  bool get_pd_position(encl_id_t &eid, slot_id_t &sid);
  bool find_ctrlid_by_hostid();
  bool get_device_interface_type();
};

linux_ps3stor_device::linux_ps3stor_device(smart_interface *intf,
  const char *dev_name, unsigned int tgt, unsigned int ctrlId)
 : smart_device(intf, dev_name, "ps3stor", "ps3stor"),
   ...
   m_host(0),
   m_dev_interface(PS3STOR_PD_INTERFACE_TYPE_SAS),
   m_nsd(0xffffffffU)
...
smart_device * linux_ps3stor_device::autodetect_open()
{
  ...
  // Use INQUIRY to detect type
  {
    ...
    ata_device * newdev = smi()->autodetect_sat_device(this, req_buff, len);
    if (newdev) {// NOTE: 'this' is now owned by '*newdev'
      m_dev_interface = PS3STOR_PD_INTERFACE_TYPE_SATA;
      return newdev;
    }
  }
...
bool linux_ps3stor_device::open()
{
  if(!m_open_flag) {
    ...
    //find ctrlid if not input
    if(m_cid == PS3STOR_INVALID_CTRL_ID) {
#ifdef PS3STOR_SUPPORT_DEV_SDX
      if(!find_ctrlid_by_hostid()) {
...
bool linux_ps3stor_device::scsi_cmd(scsi_cmnd_io *iop)
{
 // Controller rejects Test Unit Ready
  ...
  if(iop->cmnd_len > PS3LIB_SCSI_MAX_CDB_LENGTH) {
    return set_err(ENOSYS, "ps3stor cannot support scsi cdb longer than %d", PS3LIB_SCSI_MAX_CDB_LENGTH);
  }

  //use ps3lib to get enclid slotid
  ...
  memset(scsi_passthru, 0, data_size);
...
bool linux_ps3stor_device::get_pd_position(encl_id_t &eid, slot_id_t &sid)
{
  ...
  if(m_eid == PS3STOR_INVALID_ENCL_ID && m_sid == PS3STOR_INVALID_SLOT_ID) {
    ps3stor_errno err = ps3libPdBaseInfoGetByDevId(m_cid, m_did, &baseinfo);
    if(err != PS3STOR_ERRNO_SUCCESS) {
      //todo print invalid deviceid or ctrlId
      return false;
    }
...
bool linux_ps3stor_device::find_ctrlid_by_hostid()
{
#ifdef PS3STOR_SUPPORT_DEV_SDX
  ps3stor_ctrl_list_t ctrl_list = {};
  ps3stor_errno err = ps3libCtrlListGet(&ctrl_list);
  if(err != PS3STOR_ERRNO_SUCCESS) {
    //todo print err msg
    return false;
  }
  ...
  for(U16 i = 0; i < ctrl_list.count; i++)
  {
    ...
    if(pcieinfo.hostId == m_host) {
      m_cid = ctrl_list.ctrlId[i];
      return true;
    }
  }
#endif
...
bool linux_ps3stor_device::get_device_interface_type()
{
  ps3stor_pd_baseinfo_t baseinfo = {};
  //1.get baseinfo from ioc 
  ps3stor_errno err = ps3libPdBaseInfoGetByDevId(m_cid, m_did, &baseinfo);
  if(err != PS3STOR_ERRNO_SUCCESS) {
    //todo print invalid deviceid or ctrlId
    return false;
...
private:
  ...
  int ps3stor_pd_add_list(unsigned int ctrlid, smart_device_list & devlist);

  static bool s_ps3stor_init; //flag for init ps3lib
  ps3stor_ctrl_list_t m_ctrl_list;
  bool ps3stor_init_lib();
  bool ps3stor_check_ctrlid_exist(unsigned ctrlid); //call after init_ps3stor_lib
  bool ps3stor_find_ctrlid(unsigned hostid, ctrl_id_t &cid);
};

bool linux_smart_interface::s_ps3stor_init = false;
...
linux_smart_interface::~linux_smart_interface()
{
  if(s_ps3stor_init) {
    ps3libDestory();
    s_ps3stor_init = false;
  }
}
...
bool linux_smart_interface::get_dev_ps3stor(smart_device_list &devlist)
{
  ...
  {
    ps3stor_pd_add_list(m_ctrl_list.ctrlId[i], devlist);
  }

  return true;
}

int linux_smart_interface::ps3stor_pd_add_list(unsigned int ctrlid, smart_device_list &devlist)
{
  //1. get pd list by ps3lib
  ps3stor_errno        err            = PS3STOR_ERRNO_SUCCESS;
  ps3stor_devid_list_t pd_devid_list  = {};

  err = ps3libPdDevIdListGetByCtrlId(ctrlid, &pd_devid_list);
  if(err != PS3STOR_ERRNO_SUCCESS) {
    // TODO print
    return 0;
  }
...
bool linux_smart_interface::ps3stor_init_lib()
{
  //init ps3lib and get controller list , return false on err or not found
  if(!s_ps3stor_init) {
  ...
  return s_ps3stor_init;
}

bool linux_smart_interface::ps3stor_check_ctrlid_exist(unsigned ctrlid)
{
  for(unsigned i = 0; i < m_ctrl_list.count; i++) {
    if(ctrlid == m_ctrl_list.ctrlId[i])
      return true;
  }
  return false;
}

bool linux_smart_interface::ps3stor_find_ctrlid(unsigned hostid, ctrl_id_t &cid)
{
 // init ps3lib and get controller list , return false on err or not found
  if(!ps3stor_init_lib()) {
    return false;
  ...
  for(U16 i = 0; i < m_ctrl_list.count; i++)
  {
    ...
    if(pcieinfo.hostId == hostid) {
      cid = m_ctrl_list.ctrlId[i];
      return true;
    }
  }
  return false;
...
smart_device * linux_smart_interface::get_custom_smart_device(const char * name, const char * type)
{
  ...
  if (sscanf(type, "ps3stor,%d", &disknum) == 1) {
    ...
    if(ps3stor_init_lib()) {
      ...
      pout("get_custom_smart_device: ps3stor_init_lib failed.\n");
      set_err_np(EINVAL, "get_custom_smart_device: ps3stor_init_lib failed.\n");
    }
